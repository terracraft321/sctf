// Added new techs. Also changed 2 boats for longboat factories.
...
void SetupScrolls(CRules@ this)
{
	...
	//new
	{
		ScrollDef def;
		def.name = "Exchange";
		def.scrollFrame = FactoryFrame::storage;
		def.level = 0.0f;
		def.tier = 1.0f;
		def.timeSecs = t * 5000;
		all.scrolls.set("exchange", def);
	}
...

	{
		...
		def.connections.push_back("dropping");//added
...

	{
		...
		def.connections.push_back("mounted_bow");//changed
...

	{
		...
		def.tier = -0.5f;
		def.timeSecs = t * 360;
		def.connections.push_back("lightballista");//changed
		addScrollItemsToArray("Longboat", "longboat", 	m * 60, true, 2, @def.items);//changed
	...
	{
		ScrollDef def;
		def.name = "Dropping Weapones";
		def.scrollFrame = FactoryFrame::boulder;
		def.level = 2.0f;
		def.tier =  0.0f;
		def.timeSecs = t * 240;
		def.connections.push_back("bomber");
		addScrollItemsToArray("Boulder", "boulder", 	m * 15, false, 1, @def.items);
		addScrollItemsToArray("Bomb Ball", "bombball", 	m * 30, false, 1, @def.items);
		all.scrolls.set("dropping", def);
	}
...

	{
		...
		all.scrolls.set("mounted_bow", def);//changed
	}
	...
	{
		ScrollDef def;
		def.name = "Light Ballista";
		def.scrollFrame = FactoryFrame::factory;
		def.level = 3.0f;
		def.tier = -0.5f;
		def.timeSecs = t * 230;
		def.connections.push_back("ballista");
		addScrollItemsToArray("Light Ballista", "lightballista", 	m * 60, true, 2, @def.items);
		addScrollItemsToArray("Ballista Bolts", "mat_bolts", 	m * 60, false, 1, @def.items);
		all.scrolls.set("lightballista", def);
	}
	...
	//new
	{
		ScrollDef def;
		def.name = "Bomber";
		def.scrollFrame = FactoryFrame::unknown;
		def.level = 4.0f;
		def.tier = 0.0f;
		def.timeSecs = t * 1000;
		addScrollItemsToArray("Bomber", "bomber", 	m * 120, true, 1, @def.items);
...

	{
		...
		addScrollItemsToArray("Molotov Cocktails", "mat_molotov", 	m * 10, false, 3, @def.items);
...

	{
		...
		addScrollItemsToArray("Bomb Satchels", "bomb_satchel", 	m * 10, false, 1, @def.items);
	...
	copyFrom(all.scrolls, "dropping", crappy.scrolls);
...
//	copyFrom( all.scrolls, "saw", medium.scrolls );
	...
	copyFrom(all.scrolls, "lightballista", medium.scrolls);
	copyFrom(all.scrolls, "mountede_bow", medium.scrolls);
	...
	copyFrom(all.scrolls, "exchange", super.scrolls);
